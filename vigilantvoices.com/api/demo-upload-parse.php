...
function vvIngestDocument(string $sessionId, string $docId, string $title, string $text, array $meta): int {
...
for ($i = 0; $i < count($names); $i++) {
    ...
    $type = vvTypeFromName($name);
    $preview = vvReadPreview($tmp, $name);
    $field = 'Content';
    $docId = 'upload_' . time() . '_' . $i . '_' . bin2hex(random_bytes(3));
    $text = "File: {$name}\nType: {$type}\nField: {$field}\nPreview: {$preview}";
    $addedChunks = vvIngestDocument($sessionId, $docId, $name, $text, ['type' => $type, 'source' => 'upload']);
