#!/usr/bin/env python3
"""IndexNow submission utility for truenorthinsurancesolutions.com.
...
API_KEY = "b7f2a1d4-9c3e-4a8b-bf6e-7d2c5a91e8f0"
...
def run(dry_run: bool, max_retries: int, retry_backoff_seconds: int, force: bool) -> int:
    ...
    if not force and not dry_run and previous_hash and previous_hash == current_hash:
        print("URL set unchanged from previous successful run; skipping submission.")
        append_log(
            [
                {
                    "timestamp": utc_now_iso(),
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Submit truenorthinsurancesolutions.com URLs to IndexNow")
    parser.add_argument(
        "--dry-run",
        action="store_true",
